#!/usr/bin/env python
"""
Monitor and resume batch processing for enhanced embeddings generation.
"""
...
from batch_generate_embeddings_with_details import EnhancedEmbeddingGenerator

def main():
    # Check for batch ID file
    batch_id_file = Path('embeddings_batch_enhanced/current_batch_id.txt')
    
    if not batch_id_file.exists():
        print("❌ No embeddings batch ID file found. Run batch_generate_embeddings_with_details.py first.")
        return
    
    with open(batch_id_file, 'r') as f:
        batch_id = f.read().strip()
    
    print(f"📊 Monitoring enhanced embeddings batch: {batch_id}")
    ...
    try:
        ...
        if batch.status == 'completed':
            ...
            print("\n💡 Note: Embedding batches typically complete within 1-2 hours")
