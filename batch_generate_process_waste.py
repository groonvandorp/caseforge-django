#!/usr/bin/env python
"""
Batch generation of process waste analysis (TIMWOOD+) using OpenAI Batch API.

This script analyzes business processes for waste using the TIMWOOD framework and modern extensions.
It works alongside existing batch_generate_process_details.py and batch_generate_usecase_candidates.py.

This script:
1. Loads all nodes with existing process_details documents
2. Analyzes each process for various types of waste (TIMWOOD + digital/knowledge/compliance/communication)
3. Generates structured waste identification and recommendations
4. Stores results in database as NodeDocument records with document_type='process_waste'
...
class ProcessWasteBatchGenerator:
    def __init__(self, model_key='cross_industry'):
        self.model_key = model_key
        self.client = None
        self.admin_user = None
        self.model_version = None
        self.openai_model = None
        self.output_dir = Path('batch_process_waste')
    ...
    def get_nodes_with_process_details(self):
        ...
        for node in all_nodes:
            ...
            if process_details:
                # Check if process waste analysis already exists
        ...
        print(f"✅ Found {len(nodes_with_details)} nodes with process details (no process waste analysis yet)")
...
Format your response as structured markdown with clear sections and actionable insights."""
    ...
    def prepare_batch_file(self, nodes_with_details):
        ...
        input_file = self.output_dir / f'batch_input_process_waste_{self.model_key}_{timestamp}.jsonl'
    ...
    def submit_batch(self, input_file):
        ...
        batch = self.client.batches.create(
            ...
            metadata={
                "description": f"Process waste analysis for {self.model_key}",
                "type": "process_waste",
    ...
    def process_results(self, batch):
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_path = self.output_dir / f'batch_output_process_waste_{self.model_key}_{timestamp}.jsonl'
            ...
            for line in f:
                ...
                if response['status_code'] == 200:
                    ...
                    if existing:
                        ...
                        existing.save()
                        print(f"📝 Updated process waste analysis for: {node.code} - {node.name}")
                    else:
                        # Create new document
                        NodeDocument.objects.create(
                            node=node,
                            document_type='process_waste',
                            title=f"Process Waste Analysis (TIMWOOD+) - {node.name}",
                            content=content,
                        ...
                        print(f"✅ Created process waste analysis for: {node.code} - {node.name}")
    ...
    def run(self, auto_poll=True):
        ...
        print("🚀 Starting Process Waste Analysis Batch Generation")
        print("=" * 50)
        ...
        response = input(f"\nGenerate process waste analysis for {len(nodes_with_details)} nodes? (y/n): ")
        if response.lower() != 'y':
            print("❌ Cancelled")
            return
        ...
        if auto_poll:
            ...
            print(f"   python monitor_process_waste.py")
...
def main():
    ...
    parser.add_argument('--model', default='cross_industry', help='Model key (default: cross_industry)')
    parser.add_argument('--no-poll', action='store_true', help='Submit without polling')

    args = parser.parse_args()

    generator = ProcessWasteBatchGenerator(model_key=args.model)
