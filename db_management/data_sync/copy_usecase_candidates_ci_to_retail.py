#!/usr/bin/env python3
"""
Copy AI usecase candidates from Cross Industry to Retail model using PCF ID-based matching.
...
def get_copyable_usecase_candidates(matches):
    ...
    for match in matches:
        ...
        if ci_candidates.exists():
            # Check if Retail node already has usecase candidates
            retail_candidates = NodeUsecaseCandidate.objects.filter(node=match['retail_node'])
            
            if not retail_candidates.exists():
                copyable.append({
                    **match,
...
def copy_usecase_candidates(copyable_matches, dry_run=True):
    """Copy usecase candidates from CI to Retail nodes."""
            ...
            for i, match in enumerate(copyable_matches):
                try:
                    ...
                    for candidate in match['ci_candidates']:
                        try:
                            # Generate new unique UID for the copied candidate (avoid conflicts)
                            new_uid = f"ret_{str(uuid.uuid4())[:8]}_{candidate.candidate_uid}"
...
def verify_copy_results():
    ...
    retail_count = NodeUsecaseCandidate.objects.filter(
        node__model_version__model__model_key='apqc_pcf_retail'
    ).count()
    
    print(f"   Cross Industry usecase candidates: {ci_count}")
    print(f"   Retail usecase candidates: {retail_count}")
    
    return ci_count, retail_count

def main():
    """Main execution function."""
    print("🚀 PCF ID-BASED AI USECASE CANDIDATES COPY: CI → RETAIL")
    print("=" * 60)
